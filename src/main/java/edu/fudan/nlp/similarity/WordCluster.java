...
public class WordCluster implements Serializable {
    ...
    private void statisticProb() {
        ...
        while (it.hasNext()) {
            ...
            it.setValue(v);
            Cluster cluster = new Cluster(it.key(), v, alpahbet.lookupString(it
                    .key()));
    ...
    protected void merge(int c1, int c2) {
        ...
        // 修改cluster
        Cluster cluster = new Cluster(newid, clusters.get(c1),
                clusters.get(c2), pc);
    ...
    public Cluster startClustering() {
        ...
        for (round = 0; round < Math.min(slotsize, mergeCount); round++) {
            slots.add(idx[round]);
            System.out.println(round + "\t" + alpahbet.lookupString(idx[round])
                    + "\t" + slots.size());
        ...
        while (slots.size() > 1) {
            if (round < mergeCount)
                System.out.println(round + "\t"
                        + alpahbet.lookupString(idx[round]) + "\tSize:\t"
                        + slots.size());
            else
