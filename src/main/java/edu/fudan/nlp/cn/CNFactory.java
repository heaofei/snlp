...
public class CNFactory {
	 ...
	 * @throws LoadModelException 
	 */
	public static void addDict(ArrayList<String[]> al) throws LoadModelException {
		dict.add(al);
		setDict();
	}

	private static void setDict() throws LoadModelException {
	...
	public DependencyTree parse2T(String[][] tagg) {
		if(parser==null)
			return null;
		return parser.parse2T(tagg[0], tagg[1]);
	}
	...
	public DependencyTree parse2T(String sent){
		...
		if(wc==null)
			return null;
		return parser.parse2T(wc[0], wc[1]);
	}
	/**
	 * 实体名识别
	 * @param s
